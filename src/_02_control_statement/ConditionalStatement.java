...
//        if(name == "김코딩"){// 문자열 비교시 , 비교 연산자 제대로 된 비교가 안됨.
//            System.out.println("환영 합니다");
//        }else {
//            System.out.println("이름을 다시 입력 해주세요!");
//        }
        if (name.equals("김코딩")) {// 문자열: 비교 연산자 말고 equals()로 검사
            System.out.println("환영 합니다");
        } else {
            System.out.println("이름을 다시 입력 해주세요!");
        }
        ...
        // - '.equals()' 메서드 : 두 객체의 내용이 돌인한지 비교

        // "문자열 리터럴"의 경우 java 에서 특별 취급
        // 동일한 문자열 리터럴이 사용되는 경우, java 컴파일러는 문자열 풀(string pool)이라는 공유된 메모리
        //str1, str2 같은 경우에 같은 문자열을 가리키고 있으므로 같은 참조 값을 가짐
        String str1 = "hello";
        String str2 = "hello";
        if (str1 == str2) {
        ...
        //문자열을 "동적 할당"하게 되는 경우, new String(...)을 이용해서 새로운 문자열 객체를 생성 -> 서로 다른 객체를 가리킴
        ...
        /*switch(day){
            ...
            case 7:
                ...
                break;
            default:
                dayOfWeek = "잘못된 입력 입니다";
                break;
        }*/

        //java 11부터 switch 문이 향상된 switch 문으로 변경됨. 기존 방식으로 사용해도 무방
